...
| 버전 | 날짜 | 요약 |
|------|------|------|
| v0.1.15 | 2026-08-17 | 그룹 카드에 스캔 임계값 대신 측정된 유사도 점수를 표시하고, 디스크에서 사진 파일이 변경되면 재스캔 시 오래된 썸네일을 자동으로 갱신합니다. |
| v0.1.14 | 2026-08-17 | 디스크에서 사진 파일이 변경되면 재스캔 시 오래된 썸네일을 자동으로 갱신하여 화면의 미리보기와 중복 그룹핑 결과가 일치하도록 수정했습니다. |
| v0.1.13 | 2026-08-17 | 복구 가능한 2단계 격리 이동, 중단 작업의 시작 시 복구, 보호된 격리 보관 위치로 사진 정리 안전성을 강화하고 앱 재개 후 토스트 만료를 바로잡았습니다. |
| v0.1.12 | 2026-07-25 | 앱 재시작 시 이미 분류한 그룹이 다시 표시되던 문제를 수정하고, 시작 스냅샷 응답 직전에 현재 완료 상태와 사진 마크를 덮어씁니다. |
| v0.1.11 | 2026-07-25 | 시작 직후 오래된 그룹이 잠시 나타나는 문제를 수정하고, 최신 그룹을 불러오는 동안 로딩 상태를 명확히 표시하며, 정리 후 시작용 스냅샷을 갱신합니다. |
| v0.1.10 | 2026-07-25 | 기능 변경 없이 v0.1.9에서 도입된 백그라운드 자동 업데이트 흐름의 실동작을 검증하기 위한 릴리스입니다. |
